...
public class LevelLoader {
    ...
    private ReadCSVFile level5Floor, level5Walls, level5Spawns;

    private TileMap tileMap1, tileMap2, tileMap3, tileMap4, tileMap5;
    public Level level, level1, level2, level3, level4, level5;

    public LinkedList<Level> levelList;

    public int currentLevel;

    // TODO: This can really easily be simplified, there is a lot of repetition, especially in the switch cases
    public LevelLoader(Handler handler) throws IOException {
        ...
        this.level5Spawns = new ReadCSVFile("src/main/java/core/levels/Credits_Spawns.csv");
        this.level5Walls = new ReadCSVFile("src/main/java/core/levels/Credits_Walls.csv");
        this.level5Floor = new ReadCSVFile("src/main/java/core/levels/Credits_Floor.csv");
        this.tileMap5 = new TileMap(dungeon1Sheet);
        this.level5 = new Level(tileMap5, level5Floor, level5Walls, level5Spawns);
        this.levelList.add(this.level5);
    ...
    public void loadLevel(Level level){
        ...
        for (String[] row : level.floorFile.rows) {
            ...
            for (String item: row) {
                ...
                switch (item) {
                    case "0":
                        floor = new Floor(xx, yy,  level.tileMap.tiles.get(item), BlockID.floor);
        ...
        for (String[] row : level.wallFile.rows) {
            ...
            for (String item : row) {
                ...
                switch (item) {
                    case "15":
                        wall = new Wall(xx, yy, level.tileMap.tiles.get(item), BlockID.wall, 0, 0);
                        wall.setBlock_image(level.tileMap.tiles.get(item).grabImage(1, 4, 64, 64));
                        handler.addObject(wall, handler.walls);
                        break;
                    case "16":
                        wall = new Wall(xx, yy, level.tileMap.tiles.get(item), BlockID.wall, 44, -44);
                        wall.setBlock_image(level.tileMap.tiles.get(item).grabImage(2, 4, 64, 64));
                        handler.addObject(wall, handler.walls);
                        break;
                    case "17":
                        wall = new Wall(xx, yy, level.tileMap.tiles.get(item), BlockID.wall, 0, -44);
